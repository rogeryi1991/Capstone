...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        # Group by Launch Site to count successful launches per site
        ...
        # Count success and failure launches
        success_failure_counts = filtered_df['class'].value_counts().reset_index(name='count')
        success_failure_counts['class_name'] = success_failure_counts['class'].apply(lambda x: 'Success' if x == 1 else 'Failure')
        success_failure_counts.columns = ['class', 'count', 'class_name']

        fig = px.pie(success_failure_counts, values='count', 
                     names='class_name', 
